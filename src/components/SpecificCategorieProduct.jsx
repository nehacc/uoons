...
const SpecificCategorieProduct = ({ data, sortOrder, limit, priceRange }) => {
  ...
  .filter(product => 
    product.product_sale_price >= priceRange[0] && product.product_sale_price <= priceRange[1]
  )
  .sort((a, b) => {
    if (sortOrder === 'asc') {
  ...
  .slice(0, limit); // Limit the number of products displayed
